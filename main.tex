...
\par
Finkensiep suggests in his thesis that the proto-voice model may be an effective way to infer higher level latent entities such as harmonies. Thus, in this project I will answer the question: is this model an effective way to annotate harmonies? By ‘effective’ we are referring to two things:
\begin{itemize}
...
The Protovoice model is concerned with the analysis of Western Classical music, although it could be adapted to different musical styles. For the purposes of this dissertation I will make music theoretical assumptions, but their justification is left to the appendix.
\par
...
\par
The notion of a \textit{voice} is crucial for the understanding of the protovoice model. A voice typically refers to a single melodic line (sequence of notes) that is part of a polyphonic (multiple voices) musical composition. The use of the word is derived from it's use in Bach's four-voice chorales, which consist of 4 sung melodic lines. The term voice is used is used more generally however, the melodic lines do not need to be sung or voice-like in character and can be performed by any melodic instrument. 
\par
A piece of music, Explain polyphony and monophony here?
\par
Little diagram of a piece of music, with voices explicitly marked.
\par
Three musical concepts that the protovoice model makes use of:

\begin{itemize}
  \item \textbf{Voice-leading}: What are voices? Implicit vs Explicit voices. Define Stepwise
  \item \textbf{Harmony}: notes that perceived as sounded simulatiously cause an emergent cognitive phenomonon of harmony, wherein the way the collection of notes sounds together depends on the pitch relationship between notes.
  \item \textbf{Functional Dependencies}: Repetition/ ornamentation.
\end{itemize}
\par 
Insert Diagram demonstrating all three phenoma given a simple example with lines.
\par
...
\par
The protovoice model is a generative model on the note level of a piece of music, which represents a piece of music as a graph where each note is a node, and notes are connected by stepwise protovoice edges. It generates a piece of music through sequential operations on notes, inserting new notes with edges connected to existing notes.
...
\par 
\subsubsection{Inner Operations} % (fold)
\label{sub:Inner Operations}

A \textit{protovoice} is a sequence of edges between notes based on functional dependencies. The protovoice model is characterised by 3 primitive generative operations on notes.
\par 
...
% subsection Inner Operations (end)

\subsubsection{Outer Operations}

To model simulataneity of notes we introduce slices, representing segments of a piece where a group of notes are heard. These provide a higher level abstraction that are used to capture more musical structure, which we refer to as \textit{outer structure}
\par 
