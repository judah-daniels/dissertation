...
The datasets used contain a selection or corpuses, bodies of work by a single conposer as shown below, these were selected to exhibit a range of different styles. 

\subsubsection{Evaluation Plan}
Split into a development set and a hold out validation set.
Hyper parameters are chosen using 10 fold cross validation.
Evaluation is done using the hold out validation set. 
This allows assessment of how well the model predicts \textbf{unseen} inputs.
...
\begin{equation}
  \begin{gathered}
    ...
    &$where $\bm{\phi}_{l}^{(p)} = P($Pitch $=c | $Label $=l)\\ 
...
Given the path graph is linear we treat the surface as a string. 

And apply redution ooperations, reversing the production rules.
