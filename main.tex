...
\FloatBarrier
\par
Polyphony refers to a piece of music that can contains more than one voice. Typically polyphonic music will have a set number of voices throughout the piece, but \textit{free polyphony} refers to music where the number of voices is arbitrary and can change throughout the piece.
\par
...
\begin{figure}
  ...
  \captionsetup{width=.9\linewidth}
  \caption{A short cadential phrases with two voices.}
...
\begin{figure}
  ...
  \captionsetup{width=.9\linewidth}
  \caption{Chord labels for the short cadential phrase.}
...
\begin{equation}
  ...
  \end{align} 
\end{equation}
% subsubsection Inner Structure (end)
...
\FloatBarrier
% subsubsection Outer Structure (end)
...
\begin{equation}
  D_{opt} = \argmax_{D} \left( p(D|\text{surface})\right)
...
\begin{itemize}
  ...
  \item \textbf{Ambiguity}: The notes in a given segment may not be enough to determine the chord label, as context is vital. For example, the D and B in Figure~\ref{fig:pvHarmony}. Furthermore, ambiguity is often used with much license to create artistic interest.
  \item \textbf{Non chord-tones}
\end{itemize}

Difficulty of inferring harmony: ambiguity, and ornamentation, segmentation. 
